local a, b = pcall(function()
	local Output = print -- require(script.Parent.Parent:WaitForChild('Modules'):WaitForChild('Garbadge_Collect')).create --// Function used to output remote calls (Change to rconsoleprint to use Synapse's console)
	...
	do --// Save remote calls and settings to their files
		...
		if GotSettingsJSON then
			...
			if GotSettings then
				...
				warn("Loaded settings from file")
			else
				warn("Could get saved settings file, but couldn't decode from JSON")
			end
		else
			warn("Couldn't get saved settings file")
		end
		warn("Settings: " .. Parse(Settings) .. "\n---------------------")
